...
> **Provenance.** This document, and the mechanisms it describes, were produced on the
> `cross-run-memory-and-finalization-fixes` branch by Sean Koh. It is reproduced here unaltered
> because its findings stand on their own; the two branches were **not** merged before the
> deadline, so parts of it describe code that is not in this tree. Specifically,
> `ResearchLineageLedger` and `research.proposal_breadth` do **not** exist on this branch;
> `_proposal_family_is_blocked`, `_proposal_family` and `OuterQueryLedger` do. The campaign
> counts, admission records and measurements below come from that branch's runs, not from the
> `maki-overnight-*` series reported in [`RESULTS.md`](RESULTS.md) section 3.5.
